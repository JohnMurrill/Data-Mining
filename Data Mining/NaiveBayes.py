...
# P(wod | spam) for each possible word in the file. isSpam is a boolean variable
# to manipulate the function for the NotSpam and IsSpam cases.
def ProbBGivenA(A,B,isSpam):
        numMeans=[0]*len(B[0])
...
# Given a matrix of evidence, this function calculates the P(spam | word) for each word.
# It returns a 1D 57 element matrix
...
# Gets the total probability that an email is spam given all words that appear or do not appear in the email.
# Returns a single
...
# Splits the dataset and the corresponding class data into a series of N partitions provided with the numPartitions
# variable.
...
# Given a series of N-1 folds, this function trains the model on the provided
# N-1 training folds and tests the model against the remaining
# holdout fold. It prints the accuracy of the model against each fold, and returns
# a single dimensional list of the accuracies in order.
def TrainOnFolds(dataset,answerSet, holdout, holdoutAnswers):
        ...
        for i in range(len(holdout)):
                isSpamProb = BayesAllWords(BayesEachWord(ProbBGivenA(newAnswerSet,newDataSet,True),ProbSpam(newAnswerSet,newDataSet),ProbWord(newAnswerSet,newDataSet)),holdout[i])
                notSpamProb = BayesAllWords(BayesEachWord(ProbBGivenA(newAnswerSet,newDataSet,False),ProbSpam(newAnswerSet,newDataSet),ProbWord(newAnswerSet,newDataSet)),holdout[i])
                if isSpamProb > notSpamProb:
                        classifications[i] = 1
                else:
                        classifications[i] = 0
...
# Main method. Loads the appropriate data files, partititions the data (into 10 folds initially),
# and runs the model. The final print statement
# outputs the overall accuracy of the model.
def main():
        filename = 'spamdata_binary.txt'
        #Number of folds.
        splitRatio = 10
        dataset = LoadWordData(filename)
        filename='spamlabels.txt'
        answerSet = LoadWordData(filename)
        trainSet,newAnswerSet = SplitDataSet(dataset,answerSet,splitRatio)
        newObservation=dataset[1000]
        print(BayesAllWords(BayesEachWord(ProbBGivenA(answerSet,dataset,True),ProbSpam(answerSet,dataset),ProbWord(answerSet,dataset)),newObservation))
        print(BayesAllWords(BayesEachWord(ProbBGivenA(answerSet,dataset,False),1-ProbSpam(answerSet,dataset),ProbWord(answerSet,dataset)),newObservation))
        print(len(trainSet),len(newAnswerSet))
